...
class AutoClicker:
    ...
    def __init__(self, master):
        ...
        self.mouse_button_index = ttk.Combobox(self.mouse_button_frame, width=15, textvariable=tk.StringVar(),
                                               state="readonly")
        ...
        self.key_start_index['values'] = (
        keyboard.Key.f1, keyboard.Key.f2, keyboard.Key.f3, keyboard.Key.f4, keyboard.Key.f5, keyboard.Key.f6,
        keyboard.Key.f7, keyboard.Key.f8, keyboard.Key.f9, keyboard.Key.f10, keyboard.Key.f11, keyboard.Key.f12)
        ...
        self.timer_label = ttk.Label(self.master, text="Time elapsed: 0s")
        self.timer_label.pack()
        self.click_count_label = ttk.Label(self.master, text="Click count: 0")
    ...
    def update_timer(self):
        ...
        self.timer_label.config(text="Time elapsed: {}s".format(elapsed_time))
    ...
    def on_press(self, key):
        if key == self.user_defined_start and self.break_loop:
            ...
            self.update_timer()
            self.click_loop()
        if key == self.user_defined_stop and not self.break_loop:
